...
export const switchBaseProps = {
  ...
  checkedIcon: props.ofType<IconProperty | null>().default(() => CheckboxFilled),
...
export interface SwitchBaseScopedSlots {
  label?: { checked: boolean; indeterminate: boolean; focus: boolean; disabled: boolean };
}
...
export default defineComponent<SwitchBaseOutsideProps, SwitchBaseEvents, SwitchBaseScopedSlots, SwitchBaseMethods>('switch-base')
  .create({
    props: {
      baseName: props(String).required,
      role: props.ofStringLiterals('radio', 'checkbox').required,
      ...switchBaseProps,
    },
    computed: {
      tabIndex() {
    ...
    render(h) {
      ...
      let label = $scopedSlots.label
        ? $scopedSlots.label({ checked, indeterminate, disabled, focus: this.isFocused })
        : (
          $slots.default
          &&
          <span class={root.element('label')}>
            {$slots.default}
          </span>
        );
      return h(
