import type { EsModule, ImportedComponent } from 'vue/types/options';
import type {
  LazyloadScopedSlots,
  LazyloadEvents,
  LazyloadTransitions,
...
const Lazyload = defineComponent<
  LazyloadProps,
  LazyloadEvents,
  LazyloadScopedSlots
>('lazyload').create({
  props: {
    ...
    asyncComponent: props
      .ofType<(() => Promise<ImportedComponent>) | null>()
      .default(null),
    imgAttrs: props.ofType<ImgHTMLAttributes>().optional,
    imgStyle: props.ofType<CSSProperties>().optional,
  ...
  methods: {
    ...
    loadAsync() {
      if (this.asyncComponent) {
        this.asyncComponent()
          .then((component: EsModule<any>) => {
            this.$emit('loaded');
            this.asyncNode = this.$createElement(
