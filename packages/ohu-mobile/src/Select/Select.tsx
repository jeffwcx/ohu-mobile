...
export default defineComponent<
  ...
  SelectInnerProps
>('select')
  .mixin(fieldMixin('stateValue', 'value', true))
  .create({
    model: {
      prop: 'value',
      event: 'change',
    ...
    data() {
      const currentValue = this.getCurrentStateValue(
      ...
      return {
        ...
        unconfirmSelectedOption: undefined as
          | SelectOption
          | undefined
          | SelectOption[],
        delay: false,
    ...
    methods: {
      ...
      handleChange(value: any, option: SelectOption | SelectOption[]) {
        if (this.shouldConfirm) {
          ...
          this.$emit(
            'select',
            this.unconfirmStateValue,
            this.unconfirmSelectedOption,
          );
          return;
        }
            ...
            {this.options.map(({ label, value, disabled }) => {
              return (
                <option
                  disabled={!!disabled}
                  value={label}
                  selected={value === this.stateValue}
                >
                  {value}
                </option>
              );
      ...
      renderPopup(popupClass: BlockContext, contentStyle: CSSProperties) {
        ...
        const headerNodeData = {
          props: {
            ...(this.fullScreen
              ? fullScreenPopupHeaderProps
              : defaultPopupHeaderProps),
            ...this.headerProps,
        ...
        return (
          <Popup {...nodeData}>
            {this.title && (
              <Popup.Header {...headerNodeData}>{this.title}</Popup.Header>
            )}
    ...
    render() {
      ...
      return (
        <div class={root}>
          ...
          <div
            class={inputNode
              .is([outline ? 'outline' : 'normal'])
              .has([!noBorder && 'border'])}
            onClick={this.open}
            role="button"
          >
            {controlContent && <div class={inputControl}>{controlContent}</div>}
            {placeholder && !controlContent && (
              <div class={placeholderNode}>{placeholder}</div>
            )}
            {this.allowClear && this.stateValue && (
              <div
