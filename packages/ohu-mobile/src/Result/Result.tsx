...
import type { CSSProperties, VNode, VNodeChildren } from 'vue';
...
export interface ResultInnerProps {
  svgMap: ResultStatusMap;
  renderIconArea: () => VNodeChildren;
}

const Result = defineComponent<ResultProps, {}, {}, ResultInnerProps>(
  'result',
).create({
  props: {
    title: String,
  ...
  methods: {
    renderIconArea() {
      ...
      let svg: IconDef =
        statusSvg instanceof Function ? statusSvg() : statusSvg;
      if (svg) {
  ...
  render() {
    ...
    return (
      <div class={root}>
        <div class={root.element('icon')}>{this.renderIconArea()}</div>
        {title && <h2 class={root.element('title')}>{title}</h2>}
        {subTitle && <h3 class={root.element('subtitle')}>{subTitle}</h3>}
        {$slots.extra && (
