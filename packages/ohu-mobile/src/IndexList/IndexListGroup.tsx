...
import {
  IndexListGroupProps as Props,
  IndexListGroupEvents as Events,
} from './types';
...
export default createIndexListGroup.create({
  ...
  render() {
    ...
    const { index, sticky, title } = this.$props as Props;
    return (
      <div class={root}>
        <transition name={`${$prefix}scale-top`}>
          {this.subheader && this.ancestor.enableIndex && (
            <List.Subheader sticky={sticky}>{title || index}</List.Subheader>
          )}
        </transition>
        {this.$slots.default}
      </div>
