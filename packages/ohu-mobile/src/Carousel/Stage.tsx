...
import {
  computeCSSValue,
  getClientRectByDirection,
  setTransformByDirection,
} from './utils';
...
export interface CarouselStageInnerProps {
  currentAction: 'next' | 'prev' | 'goTo' | '';
  // 所有slide的偏移量，需要事先计算好
  slideOffsets: number[];
  slideSizes: number[];
  // stage样式
  ...
  prev: (autoplay?: boolean) => boolean;
  next: (autoplay?: boolean) => boolean;

  handleStageTransitionEnd: () => void;
  getAutoSize: (node: VNode) => number | undefined;
  getPagination: (stepIndex: number) => {
    slideIndex: number;
    stepIndex: number;
  };

  setStage: (isNormal?: boolean) => void;
  getTransformOffset: (isNormal?: boolean) => number;
  getSwipeOffset: (isNormal?: boolean) => {
    offsetX: number;
    offsetY: number;
    offset: number;
  };
...
export default defineComponent<
  CarouselStageProps,
  CarouselEvents,
  CarouselStageScopedSlots,
  CarouselStageInnerProps
>('carousel-stage').create({
  props: stageProps,
  computed: {
    gapValue() {
    ...
    canLoop() {
      ...
      if (loop) {
        // 下列情况不能形成loop效果
        if (
          this.slides - this.moveStepValue !==
          (this.steps - 1) * this.moveStepValue
        ) {
          loop = false;
        }
      }
  ...
  methods: {
    renderItems() {
      ...
      if (this.canLoop) {
        children = [
          ...cloneVNodes(
            children.slice(this.slides - this.perPage, this.slides),
            true,
          ),
          ...children,
      ...
      let childenNodes = children.map((node, index) => {
        ...
        return (
          <div class={this.$bem.block('carousel-item')} style={itemStyle}>
            {node}
          </div>
        );
      });
      return (
        <div
          class={this.$rootCls().is(this.direction)}
    ...
    init() {
      ...
      let children = this.$slots.default;
      if (!children || !stageWrapper) return;
    ...
    getAutoSize(node: VNode) {
      ...
      const slideSizeValue =
        this.direction === 'horizontal' ? style.width : style.height;
    ...
    next(autoplay = false) {
      ...
      if (
        this.stepIndex === this.steps - 1 &&
        (this.rewind || this.loop || autoplay)
      ) {
        success = this.goTo(0);
      } else {
        success = this.goTo(this.stepIndex + 1);
    ...
    handleEvent(
      ...
      switch (type) {
        ...
        case 'touchend':
          ...
          this.touching = false;
          if (
            Date.now() - this.swipeData.startTime < 300 &&
            this.swipeData.currentX < 0
          ) {
            parent.startPlay();
            return;
          }
  ...
  render() {
    ...
    return (
      <div {...wrapperNodeData}>
        {this.initialized ? this.renderItems() : this.$slots.default}
